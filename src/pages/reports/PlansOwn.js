...
function PlansOwn(props) {
  ...
  const handleFilterByName = (event) => {
    if (!props.plansOwnReducer.plans_charging) {
  ...
  const emptyRows =
    page > 0
      ? Math.max(
          ...
          (1 + page) * rowsPerPage -
            (filterName === ''
              ? props.plansOwnReducer.plans.length
              : props.plansOwnReducer.plans_filtered.length)
        )
      : 0;
  const isPlanNotFound =
    (filterName === ''
      ? props.plansOwnReducer.plans.length
      : props.plansOwnReducer.plans_filtered.length) === 0;
    ...
    <Page title="Plans | Human2Coach">
      <Container>
        ...
        <Card>
          ...
          {props.plansOwnReducer.plans_charging ? (
            <Spinner />
          ) : (
            <>
              {' '}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {(filterName === '' && (!userId || userId === 'ALL')
                        ? props.plansOwnReducer.plans
                        : props.plansOwnReducer.plans_filtered
                      )
              ...
              <TablePagination
                ...
                count={
                  filterName === '' && (!userId || userId === 'ALL')
                    ? props.plansOwnReducer.totalElements
                    : props.plansOwnReducer.totalElements_filtered
                }
...
const mapStateToProps = ({ plansOwnReducer, loginReducer }) => ({ plansOwnReducer, loginReducer });
...
export default connect(mapStateToProps, mapDispatchToProps)(PlansOwn);
